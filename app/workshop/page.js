...
function WorkshopContent() {
  ...
  // Initial Fetch of active jobs for the Overview tab
  useEffect(() => {
    fetchActiveJobs()
  }, [])

  // Handle QR code deep linking (when someone scans a QR code to open a job)
  useEffect(() => {
  ...
  const toggleStone = async (field, currentValue) => {
    ...
    if (error) {
      alert("Error updating stone status! Check your Supabase columns.")
      setActiveOrder({ ...activeOrder, [field]: currentValue }) // Revert on error
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 bg-gray-50 min-h-screen pb-20">
      ...
      {/* IF NO JOB IS SELECTED, SHOW THE TAB CONTENT */}
      {!activeOrder ? (
        <>
          {activeTab === 'scanner' ? (
            <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowCamera(true)} className="bg-blue-600 text-white p-4 rounded-xl border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-blue-700">
                  <Camera />
                </button>
              </div>
              ...
              {showCamera && (
                <div className="fixed inset-0 bg-black z-50 p-6 flex flex-col items-center justify-center">
                  ...
                  <p className="text-white font-black mt-6 animate-pulse uppercase tracking-widest text-center">Scan Job Bag QR Code</p>
                </div>
              )}
              
              <div className="text-center mt-10 opacity-30">
                <Package size={64} className="mx-auto mb-4"/>
                <p className="font-black text-xs uppercase tracking-tighter">Ready for Scan or Input</p>
              </div>
            </div>
          ) : (
            <div className="grid gap-3 animate-in slide-in-from-right-4 duration-300">
              {activeJobs.map(job => (
                <div 
                  ...
                  className="bg-white p-5 rounded-2xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all cursor-pointer flex justify-between items-center group"
                >
                  <div>
                    <p className="font-black text-2xl group-hover:text-blue-600">{job.vtiger_id}</p>
                    <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest">{job.client_name || 'Stock Item'}</p>
                  </div>
                  <span className={`border-2 border-black px-3 py-1 rounded-lg text-[10px] font-black uppercase shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] ${job.is_rush ? 'bg-red-500 text-white' : 'bg-yellow-300'}`}>
                    {job.current_stage}
                  </span>
                </div>
              ))}
              {activeJobs.length === 0 && <p className="text-center text-gray-400 font-bold mt-10">No active jobs in production.</p>}
            </div>
          )}
        </>
      ) : (
        /* ACTIVE WORK CARD (Shows when activeOrder is not null) */
        <div className={`bg-white border-4 p-6 rounded-[2.5rem] shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] animate-in zoom-in duration-200 ${activeOrder.is_rush ? 'border-red-600 ring-8 ring-red-50' : 'border-black'}`}>
          ...
          <div className="flex flex-col items-center justify-center bg-gray-50 border-2 border-dashed border-gray-300 rounded-3xl p-6 mb-8">
            <div className="bg-white p-3 border-4 border-black rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] mb-3">
              <QRCodeCanvas 
                value={`${typeof window !== 'undefined' ? window.location.origin : ''}/workshop?search=${activeOrder.vtiger_id}`} 
                size={160}
                level={"H"}
              />
            </div>
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Scan to Open on Mobile</p>
          </div>
          ...
          <div className="space-y-4 mb-4">
            ...
            {activeOrder.current_stage === 'QC' && !showRejectMenu && (
              <button onClick={() => setShowRejectMenu(true)} className="w-full bg-red-50 text-red-600 p-5 border-4 border-red-600 border-dashed rounded-3xl font-black text-lg flex items-center justify-center gap-2 hover:bg-red-100 transition-all">
            ...
            {showRejectMenu && (
              <div className="bg-red-50 p-6 rounded-3xl border-4 border-black animate-in slide-in-from-top-4">
                ...
                <div className="grid gap-2">
                  ...
                  <button onClick={() => setShowRejectMenu(false)} className="text-[10px] font-black uppercase text-gray-400 mt-2">Cancel</button>
                </div>
              </div>
            )}
          </div>
...
export default function WorkshopPage() {
  return (
    <Suspense fallback={<div className="p-20 text-center font-black animate-pulse uppercase tracking-widest">Loading Workshop...</div>}>
      <WorkshopContent />
    </Suspense>
  )
}