...
export default function AdminPage() {
  const [activeTab, setActiveTab] = useState('live') // 'live' or 'completed'
  const [logs, setLogs] = useState([])
    ...
    try {
      if (activeTab === 'live') {
        ...
        const { data: done, error: doneError } = await supabase
          .from('orders')
          .select('*')
          .ilike('current_stage', '%completed%') // catches "Completed", "COMPLETED", " complete "
  ...
  // Aggregate stage durations from logs for the table view
    ...
    <div className="max-w-[90rem] mx-auto p-6 space-y-8 pb-20 relative font-sans">
      ...
      {activeTab === 'live' ? (
        ...
        <div className="space-y-12 animate-in fade-in duration-500">
          ...
          {/* ACTIVITY LOG (Only shown on Live Tab) */}
          <div className="bg-white border-4 border-black p-8 rounded-[2.5rem] shadow-[8px_8px_0px_0px_black]">
            <h2 className="font-black uppercase flex items-center gap-3 mb-6 text-xl tracking-tighter"><History size={24} /> Recent Audit Log</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {logs.length === 0 ? (
                <p className="text-gray-400 font-bold uppercase text-xs">No recent activity logs.</p>
              ) : (
                logs.map((log, i) => (
        ...
        <div className="bg-white border-4 border-black rounded-[3rem] overflow-hidden shadow-[12px_12px_0px_0px_black] animate-in slide-in-from-bottom-4 duration-500">
          ...
          {/* ARCHIVE HEADER & SEARCH + REFRESH BUTTON */}
